					  ...
					  struct pkcs11_object *obj)
{
	...
	case PKCS11_CKA_EXTRACTABLE:
		return get_bool(obj->attributes, PKCS11_CKA_EXTRACTABLE);
	/* Can't be modified once set to CK_TRUE - 11 in Table 10 */
	case PKCS11_CKA_SENSITIVE:
		return !get_bool(obj->attributes, PKCS11_CKA_SENSITIVE);
	case PKCS11_CKA_WRAP_WITH_TRUSTED:
		return !get_bool(obj->attributes, PKCS11_CKA_WRAP_WITH_TRUSTED);
	/* Change in CKS_TRUSTED can only be done by SO */
	case PKCS11_CKA_TRUSTED:
				    ...
				    enum processing_func function)
{
	...
	case PKCS11_CKA_COPYABLE:
		/*
		 * TBD - Specification mentions that if the attribute value
		 * is false it can't be set to true. Reading this it looks
		 * like it should be possible to modify this attribute by
		 * C_SetAttributeValue() but this is not marked as modifiable
		 * in Table 10 neither it is mentioned that it can be modified
		 * anywhere else. So currenly assuming we can modify it.
		 */
		if (function == PKCS11_FUNCTION_MODIFY)
			/* Only allow change from CK_TRUE -> CK_FALSE */
			return get_bool(obj->attributes, PKCS11_CKA_COPYABLE);
		else
			return false;
	default:
		break;
	...
	case PKCS11_CKO_PRIVATE_KEY:
		...
		if (class == PKCS11_CKO_SECRET_KEY) {
			if (attr_is_modifiable_secret_key(req_attr, session,
							  obj))
				return true;
		}
		if (class == PKCS11_CKO_PUBLIC_KEY) {
			if (attr_is_modifiable_public_key(req_attr, session,
							  obj))
				return true;
		}
		if (class == PKCS11_CKO_PRIVATE_KEY) {
			if (attr_is_modifiable_private_key(req_attr, session,
							   obj))
				return true;
		}
		break;
						...
						enum processing_func function)
{
	...
	for (; cur < end; cur += len) {
		...
		if (cli_ref.id == PKCS11_CKA_PRIVATE &&
		    get_bool(head, PKCS11_CKA_PRIVATE)) {
